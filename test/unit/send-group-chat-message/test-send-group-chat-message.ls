...
describe 'unit/ test -- send-group-chat-message', !->
  ...
  before-each !(done)->
    ...
    utils.clean-db-and-load-fixture 'unit//send-group-chat-message', ['users', 'chats'], done
  ...
  describe '小东未登录', !->
    ...
    can '小东发送群聊信息， 返回错误结果', !(done)->
      (xiaodong) <-! socket-helper.get-client xiaodong-info
      (data) <-! xiaodong.chats-channel.emit 'send-group-chat-message', group-chat-message
  ...
  describe '小东已登录', !->
    ...
    can '小东发送群聊信息， 返回正确结果', !(done)->
      ...
      data.new-chat-message.should.not.have.property 'chatType'
      data.new-chat-message.should.not.have.property 'pushType'
    ...
    can '小东发送群聊信息， @了王瑜和伟可， 王瑜收到邀请系统消息', !(done)->
      waiter = new utils.All-done-waiter done
      waiter1 = waiter.add-waiting-function!
      waiter2 = waiter.add-waiting-function!
      ...
      (data) <-! xiaodong.chats-channel.emit 'send-group-chat-message', group-chat-message-at-users
      waiter1!

    can '小东发送群聊信息， @了王瑜和伟可, 伟可收到邀请系统消息', !(done)->
      waiter = new utils.All-done-waiter done
      waiter1 = waiter.add-waiting-function!
      waiter2 = waiter.add-waiting-function!
    ...
    can '小东发送群聊信息，@了王瑜和伟可， 柏信收到邀请系统消息', !(done)->
      waiter = new utils.All-done-waiter done
      waiter1 = waiter.add-waiting-function!
      waiter2 = waiter.add-waiting-function!
    ...
    can '小东发送群聊信息，@了王瑜和伟可， 柏信收到普通消息', !(done)->
      waiter = new utils.All-done-waiter done
      waiter1 = waiter.add-waiting-function!
      waiter2 = waiter.add-waiting-function!
      baixin.chats-channel.on 'push-new-chat-message', !(data)->
    ...
    can '小东发送群聊信息， @了王瑜和伟可， 自己收到邀请系统消息', !(done)->
      waiter = new utils.All-done-waiter done
      waiter1 = waiter.add-waiting-function!
      waiter2 = waiter.add-waiting-function!

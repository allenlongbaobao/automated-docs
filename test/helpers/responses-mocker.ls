...
mock-locations-channel-initial-response = !(location-id, current-uid)->
  (locations, interesting-points-summaries-map, recent-messages-map) <- mock-channels!
...
mock-channels = !(callback)->
  ...
  if callback
    callback locations, interesting-points-summaries-map, recent-messages-map
